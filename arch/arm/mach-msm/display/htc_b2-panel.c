					    ...
					    const struct i2c_device_id *id)
{
	...
	if (!i2c_check_functionality(adapter, I2C_FUNC_SMBUS_BYTE_DATA)) {
		pr_err("%s: Failed to i2c_check_functionality \n", __func__);
		return -EIO;
	}

	if (!client->dev.of_node) {
		pr_err("%s: client->dev.of_node = NULL\n", __func__);
		return -ENOMEM;
	}

	ret = tps_65132_add_i2c(client);
	if (ret < 0) {
		pr_err("%s: Failed to tps_65132_add_i2c, ret=%d\n", __func__,ret);
		return ret;
	}
...
static int htc_b2_regulator_init(struct platform_device *pdev)
{
	...
	if (!pwrdata) {
		pr_err("%s: failed to alloc pwrdata\n", __func__);
		return -ENOMEM;
	}
	...
	if (IS_ERR(pwrdata->vddio)) {
		pr_err("%s: couldn't get vddio reg, rc=%ld\n", __func__,
				PTR_ERR(pwrdata->vddio));
	...
	pwrdata->lcmio = of_get_named_gpio(pdev->dev.of_node, "htc,lcm_1v8-gpio", 0);
...
int htc_b2_panel_reset(struct mdss_panel_data *pdata, int enable)
{
	...
	if (enable) {
		if (pdata->panel_info.first_power_on == 1) {
			pr_info("reset already on in first time\n");
			return 0;
		}
