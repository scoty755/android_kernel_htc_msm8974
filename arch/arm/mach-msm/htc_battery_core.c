				...
				const char *buf, size_t count)
{
	...
	if (!battery_core_info.func.func_set_full_level) {
		pr_err("No set full level function!\n");
		return -ENOENT;
	}
				...
				const char *buf, size_t count)
{
	...
	if (!battery_core_info.func.func_set_full_level_dis_batt_chg) {
		pr_err("No set full level (disable battery charging only) function!\n");
		return -ENOENT;
	}
...
int htc_battery_charger_disable()
{
	...
	if (!battery_core_info.func.func_charger_control) {
		pr_err("No charger control function!\n");
		return -ENOENT;
	}
	rc = battery_core_info.func.func_charger_control(STOP_CHARGER);
	if (rc < 0)
		pr_err("charger control failed!\n");
...
int htc_battery_set_max_input_current(int target_ma)
{
	...
	if (!battery_core_info.func.func_set_max_input_current) {
		pr_err("No max input current function!\n");
		return -ENOENT;
	}
	rc = battery_core_info.func.func_set_max_input_current(target_ma);
	if (rc < 0)
		pr_err("max input current control failed!\n");
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set charger_control:%lu\n", enable);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("set context phone_call=%lu\n", phone_call);

	if (!battery_core_info.func.func_context_event_handler) {
		pr_err("No context_event_notify function!\n");
		return -ENOENT;
	}
				...
				const char *buf, size_t count)
{
	...
	pr_debug("set context play music=%lu\n", play_music);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set context network_search=%lu\n", network_search);
				...
				const char *buf, size_t count)
{
	...
	pr_debug("Set context navigation=%lu\n", navigation);
				...
				const char *buf, size_t count)
{
	...
	if (!battery_core_info.func.func_qb_mode_shutdown_status) {
		pr_err("No set trigger qb mode shutdown status function!\n");
		return -ENOENT;
	}
				  ...
				  const union power_supply_propval *val)
{
	...
	pr_debug("%s: batt power_supply_changed, psp=%d, intval=%d, ret=%d\n",
					...
					char *buf)
{
	...
	if (i < 0)
		pr_err("%s: battery: attribute is not supported: %d\n",
				__func__, off);
					...
					char *buf)
{
	...
	if (!battery_core_info.func.func_get_batt_rt_attr) {
		pr_err("%s: func_get_batt_rt_attr does not exist\n", __func__);
		return -EINVAL;
	}

	rc = battery_core_info.func.func_get_batt_rt_attr(attr_index, &val);
	if (rc) {
		pr_err("%s: get_batt_rt_attrs[%d] failed\n", __func__, attr_index);
		return -EINVAL;
	}
...
{
	pr_debug("charger control alarm is timeout.\n");
...
{
	power_supply_changed(&htc_power_supplies[BATTERY_SUPPLY]);
	pr_debug("%s: power_supply_changed: battery\n", __func__);
}
...
int htc_battery_core_update_changed(void)
{
	...
	if (battery_register) {
		pr_err("No battery driver exists.\n");
		return -1;
	}
	...
	if (battery_core_info.func.func_get_battery_info)
		...
		pr_err("no func_get_battery_info hooked.\n");
		return -EINVAL;
	}
		...
		if (htc_battery_is_support_qc20()) {
			if (CHARGER_AC == new_batt_info_rep.charging_source) {
				if (htc_battery_check_cable_type_from_usb() == DWC3_DCP) {
					power_supply_set_supply_type(&htc_power_supplies[USB_SUPPLY],
	...
	if (battery_core_info.rep.batt_temp > 680) {
		...
		if (batt_temp_over_68c_count < 3) {
			pr_debug("batt_temp_over_68c_count=%d, (temp=%d)\n",
	...
	if (test_power_monitor || (test_ftm_mode == 1)) {
		pr_debug("test_power_monitor(%d) or test_ftm_mode(%d) is set: overwrite fake batt info.\n",
	...
	if (battery_core_info.rep.charging_source <= 0) {
		if (battery_core_info.rep.batt_id == 255) {
			pr_debug("ignore invalid id when no charging_source\n");
	...
	pr_debug("ID=%d,level=%d,level_raw=%d,vol=%d,temp=%d,current=%d,"
			...
			battery_core_info.rep.batt_id, battery_core_info.rep.level,
			battery_core_info.rep.level_raw, battery_core_info.rep.batt_vol,
			battery_core_info.rep.batt_temp, battery_core_info.rep.batt_current,
			battery_core_info.rep.charging_source,
			battery_core_info.rep.charging_enabled,
			battery_core_info.rep.full_bat, battery_core_info.rep.over_vchg,
			battery_core_info.rep.batt_state, battery_core_info.rep.cable_ready,
			battery_core_info.rep.overload, battery_core_info.htc_charge_full,
			battery_core_info.rep.usb_temp, battery_core_info.rep.usb_overheat,
			dbg_cfg);

	if (is_send_batt_uevent) {
		power_supply_changed(&htc_power_supplies[BATTERY_SUPPLY]);
		pr_debug("power_supply_changed: battery\n");
	}
	if (is_send_usb_uevent) {
		power_supply_changed(&htc_power_supplies[USB_SUPPLY]);
		pr_debug("power_supply_changed: usb\n");
	}
	if (is_send_ac_uevent) {
		power_supply_changed(&htc_power_supplies[AC_SUPPLY]);
		pr_debug("power_supply_changed: ac\n");
	}
	if (is_send_wireless_charger_uevent) {
		power_supply_changed(&htc_power_supplies[WIRELESS_SUPPLY]);
		pr_debug("power_supply_changed: wireless\n");
	}
				...
				struct htc_battery_core *htc_battery)
{
	...
	if (!battery_register) {
		pr_err("Only one battery driver could exist.\n");
		return -1;
	}
	battery_register = 0;
