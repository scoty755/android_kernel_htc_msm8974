...
int htc_gauge_get_battery_voltage(int *result)
{
	...
	pr_warn("interface doesn't exist\n");
	return -EINVAL;
}
...
int htc_batt_schedule_batt_info_update(void)
{
	if (htc_batt_info.state & STATE_WORKQUEUE_PENDING) {
		...
		pr_debug("clear flag, htc_batt_info.state=0x%x\n", htc_batt_info.state);
	}
...
static void batt_lower_voltage_alarm_handler(int status)
{
	...
	if (status) {
		...
		pr_debug("voltage_alarm level=%d (%d mV) triggered.\n",
		...
		pr_debug("voltage_alarm level=%d (%d mV) raised back.\n",
			critical_alarm_level,
			htc_batt_info.critical_alarm_vol_ptr[critical_alarm_level]);
	}
...
{
	pr_debug("%s gauge event=%d\n", __func__, event);

	switch (event) {
	case HTC_GAUGE_EVENT_READY:
		if (!htc_batt_info.igauge) {
			pr_err("htc_gauge is not hooked.\n");
			break;
		}
	...
	case HTC_GAUGE_EVENT_TEMP_ZONE_CHANGE:
		if (htc_batt_info.state & STATE_PREPARE) {
			htc_batt_info.state |= STATE_WORKQUEUE_PENDING;
			pr_debug("skip due to htc_batt_info.state=0x%x\n", htc_batt_info.state);
		} else {
			pr_debug("run, htc_batt_info.state=0x%x\n", htc_batt_info.state);
			htc_batt_schedule_batt_info_update();
		}
		break;
	...
	case HTC_GAUGE_EVENT_QB_MODE_DO_REAL_POWEROFF:
		...
		break;
	default:
		pr_debug("unsupported gauge event(%d)\n", event);
		break;
	}
	return 0;
}
...
int htc_charger_event_notify(enum htc_charger_event event)
{
	...
	case HTC_CHARGER_EVENT_SRC_INTERNAL:
		...
		pr_debug("htc_ext_5v_output_now:%d\n", htc_ext_5v_output_now);
		htc_batt_schedule_batt_info_update();
#if defined(CONFIG_MACH_B2_WLJ)
		...
		queue_delayed_work(htc_batt_timer.batt_wq, &htc_batt_timer.usb_overheat_monitor_work,
				round_jiffies_relative(msecs_to_jiffies(USB_OVERHEAT_MONITOR_DELAY_MS)));
#endif
		break;
	...
	case HTC_CHARGER_EVENT_SRC_DETECTING:
		...
		queue_delayed_work(htc_batt_timer.batt_wq, &htc_batt_timer.unknown_usb_detect_work,
				round_jiffies_relative(msecs_to_jiffies(UNKNOWN_USB_DETECT_DELAY_MS)));
		break;
	...
	case HTC_CHARGER_EVENT_READY:
		if (!htc_batt_info.icharger) {
			pr_err("htc_charger is not hooked.\n");
			break;
		}
		mutex_lock(&htc_batt_info.info_lock);
	...
	case HTC_CHARGER_EVENT_SRC_CABLE_INSERT_NOTIFY:
		...
		break;
	default:
		pr_debug("unsupported charger event(%d)\n", event);
		break;
	}
	return 0;
}
...
static void cable_status_notifier_func(enum usb_connect_type online)
{
	...
	if (htc_batt_info.igauge && htc_batt_info.icharger && !(htc_batt_info.rep.batt_state))
		if (htc_batt_info.igauge->ready && htc_batt_info.icharger->ready)
			htc_batt_info.rep.batt_state = 1;

	pr_debug("%s(%d)\n", __func__, online);

	if (online == latest_chg_src && !first_update) {
		pr_debug("%s: charger type (%u) same return.\n", __func__, online);
		mutex_unlock(&cable_notifier_lock);
		return;
	}
	first_update = 0;
...
static void set_limit_charge_with_reason(bool enable, int reason)
{
	...
	if (chg_limit_active_mask & reason) {
		...
		if (prev_chg_limit_reason ^ chg_limit_reason) {
			pr_debug("chg_limit_reason:0x%x->0x%d\n",
					prev_chg_limit_reason, chg_limit_reason);
			if (!!prev_chg_limit_reason != !!chg_limit_reason &&
...
{
	pr_debug("handle context event(%d)\n", event);
...
static int htc_batt_context_event_handler(enum batt_context_event event)
{
	...
	pr_debug("context_state: 0x%x -> 0x%x\n", prev_context_state, context_state);
...
static int htc_batt_charger_control(enum charger_control_flag control)
{
	...
	pr_debug("%s: user switch charger to mode: %u\n", __func__, control);
	...
	case DISABLE_LIMIT_CHARGER:
		pr_debug("%s: skip charger_contorl(%d)\n", __func__, control);
		return ret;
		break;
	default:
		pr_debug("%s: unsupported charger_contorl(%d)\n", __func__, control);
		ret =  -1;
		break;
	}
...
static int htc_batt_ftm_charger_control(enum ftm_charger_control_flag control)
{
	...
	if (control < FTM_END_CHARGER)
		...
		pr_debug("%s: unsupported ftm_charger_contorl(%d)\n", __func__, control);
		ret =  -1;
	}
...
static void htc_batt_set_full_level(int percent)
{
	...
	pr_debug("set full_level constraint as %d.\n", percent);
...
static void htc_batt_set_full_level_dis_batt_chg(int percent)
{
	...
	pr_debug("set full_level_dis_batt_chg constraint as %d.\n", percent);
					...
					char *buf)
{
	...
	len += scnprintf(buf + len, PAGE_SIZE - len,
			"charging_source: %d;\ncharging_enabled: %d;\noverload: %d;\n"
			"Percentage(%%): %d;\nPercentage_raw(%%): %d;\nhtc_extension: 0x%x;\n",
			htc_batt_info.rep.charging_source, htc_batt_info.rep.charging_enabled,
			htc_batt_info.rep.overload, htc_batt_info.rep.level,
			htc_batt_info.rep.level_raw, htc_batt_info.htc_extension);

	if (htc_batt_info.igauge)
		if (htc_batt_info.igauge->get_attr_text)
			len += htc_batt_info.igauge->get_attr_text(buf + len, PAGE_SIZE - len);

	if (htc_batt_info.icharger)
		if (htc_batt_info.icharger->get_attr_text)
			len += htc_batt_info.icharger->get_attr_text(buf + len, PAGE_SIZE - len);
					...
					char *buf)
{
	...
	if (htc_batt_info.igauge) {
		if (htc_batt_info.igauge->get_battery_cc) {
			...
			len += scnprintf(buf + len, PAGE_SIZE - len, "cc:%d\n", cc_uah);
		}
	}
...
static int htc_batt_open(struct inode *inode, struct file *filp)
{
	...
	pr_debug("%s: open misc device driver.\n", __func__);
...
{
	pr_debug("%s: release misc device driver.\n", __func__);
	spin_lock(&htc_batt_info.batt_lock);
...
static int htc_batt_get_battery_info(struct battery_info_reply *htc_batt_update)
{
	...
	htc_batt_update->charging_source = htc_batt_info.rep.charging_source;
	htc_batt_update->charging_enabled = htc_batt_info.rep.charging_enabled;
...
static int htc_batt_get_chg_status(enum power_supply_property psp)
{
	...
	case POWER_SUPPLY_PROP_CHARGE_TYPE:
		if (htc_batt_info.icharger && htc_batt_info.icharger->get_charge_type)
			return htc_batt_info.icharger->get_charge_type();
		else
			break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_MAX:
		if (htc_batt_info.icharger && htc_batt_info.icharger->get_chg_usb_iusbmax)
			return htc_batt_info.icharger->get_chg_usb_iusbmax();
		else
			break;
	case POWER_SUPPLY_PROP_INPUT_CURRENT_SETTLED:
		if (htc_batt_info.icharger && htc_batt_info.icharger->get_chg_curr_settled)
			return htc_batt_info.icharger->get_chg_curr_settled();
		else
			break;
	case POWER_SUPPLY_PROP_VOLTAGE_MIN:
		if (htc_batt_info.icharger && htc_batt_info.icharger->get_chg_vinmin)
			return htc_batt_info.icharger->get_chg_vinmin();
		else
			break;
	case POWER_SUPPLY_PROP_INPUT_VOLTAGE_REGULATION:
		if (htc_batt_info.icharger && htc_batt_info.icharger->get_input_voltage_regulation)
...
{
	pr_debug("%s(%u sec)\n", __func__, seconds);
	mod_timer(&htc_batt_timer.batt_timer, jiffies + msecs_to_jiffies(seconds * 1000));
}
...
static void batt_regular_timer_handler(unsigned long data)
{
	if (htc_batt_info.state & STATE_PREPARE) {
		htc_batt_info.state |= STATE_WORKQUEUE_PENDING;
		pr_debug("%s(): Skip due to htc_batt_info.state=0x%x\n",
				__func__, htc_batt_info.state);
	} else {
		htc_batt_info.state &= ~STATE_WORKQUEUE_PENDING;
		pr_debug("%s(): Run, htc_batt_info.state=0x%x\n",
				__func__, htc_batt_info.state);
		htc_batt_schedule_batt_info_update();
	}
}
...
{
	pr_debug("alarm handler, but do nothing.\n");
...
static int bounding_fullly_charged_level(int upperbd, int current_level)
{
	...
	if (pingpong == 1 && upperbd <= current_level) {
		pr_debug("%s: lowerbd=%d, upperbd=%d, current=%d, pingpong:1->0 turn off\n",
				__func__, lowerbd, upperbd, current_level);
		is_input_chg_off_by_bounding = 1;
		pingpong = 0;
	} else if (pingpong == 0 && lowerbd < current_level) {
		pr_debug("%s: lowerbd=%d, upperbd=%d, current=%d, toward 0, turn off\n",
				__func__, lowerbd, upperbd, current_level);
		is_input_chg_off_by_bounding = 1;
	} else if (pingpong == 0 && current_level <= lowerbd) {
		pr_debug("%s: lowerbd=%d, upperbd=%d, current=%d, pingpong:0->1 turn on\n",
				__func__, lowerbd, upperbd, current_level);
		pingpong = 1;
	} else
		pr_debug("%s: lowerbd=%d, upperbd=%d, current=%d, toward %d, turn on\n",
...
{
	if (!htc_batt_info.icharger) {
		pr_debug("warn: charger interface is not hooked.\n");
		return;
	}

	if (htc_batt_info.icharger->is_batt_temp_fault_disable_chg)
		htc_batt_info.icharger->is_batt_temp_fault_disable_chg(&charger_dis_temp_fault);

	if (htc_batt_info.icharger->is_under_rating)
		htc_batt_info.icharger->is_under_rating(&charger_under_rating);

	if (htc_batt_info.icharger->is_safty_timer_timeout)
		htc_batt_info.icharger->is_safty_timer_timeout(&charger_safety_timeout);

	if (htc_batt_info.icharger->is_battery_full_eoc_stop)
		htc_batt_info.icharger->is_battery_full_eoc_stop(&batt_full_eoc_stop);
}

static void batt_update_info_from_gauge(void)
{
	if (!htc_batt_info.igauge) {
		pr_debug("warn: gauge interface is not hooked.\n");
		return;
	}

	if (htc_batt_info.igauge->get_battery_voltage)
		htc_batt_info.igauge->get_battery_voltage(&htc_batt_info.rep.batt_vol);

	if (htc_batt_info.igauge->get_battery_current)
		htc_batt_info.igauge->get_battery_current(&htc_batt_info.rep.batt_current);

	if (htc_batt_info.igauge->get_battery_temperature)
		htc_batt_info.igauge->get_battery_temperature(&htc_batt_info.rep.batt_temp);

	if (htc_batt_info.igauge->is_battery_temp_fault)
		htc_batt_info.igauge->is_battery_temp_fault(&htc_batt_info.rep.temp_fault);

	if (htc_batt_info.igauge->get_battery_id)
		htc_batt_info.igauge->get_battery_id(&htc_batt_info.rep.batt_id);
	...
	htc_batt_info.igauge->get_battery_soc(&htc_batt_info.rep.level_raw);
...
static void check_recharge_after_eoc(void)
{
	if (htc_batt_info.icharger->set_charger_after_eoc)
		if (batt_full_eoc_stop && htc_batt_info.rep.level_raw <= RECHARGE_LEVEL){
			pr_debug("%s: set charger to %d due to raw_level: %d%% drop\n",
...
static void check_recharge_recover(void)
{
	if (htc_batt_info.icharger->set_charger_after_eoc)
		if (htc_batt_info.rep.level_raw <= RECHARGE_RECOVER_LEVEL){
			pr_debug("%s: recover recharge due to raw_level: %d%% drop\n",
...
static void batt_check_overload(unsigned long time_since_last_update_ms)
{
	...
	pr_debug("Chk overload by CS=%d V=%d I=%d count=%d overload=%d is_full=%d\n",
			...
			is_full);
	if ((htc_batt_info.rep.charging_source > 0) && (!is_full) &&
			((htc_batt_info.rep.batt_current / 1000) > htc_batt_info.overload_curr_thr_ma)) {
...
static void batt_monitor_usb_overheat(int usb_temp)
{
	...
	if (!first && htc_batt_info.usb_temp_overheat_threshold
			&& (htc_batt_info.rep.charging_source > 0 || htc_ext_5v_output_now)) {
		if (htc_batt_info.rep.usb_temp >= htc_batt_info.normal_usb_temp_threshold) {
			if ((htc_batt_info.rep.usb_temp - pre_usb_temp ) > htc_batt_info.usb_temp_overheat_increase_threshold ||
					(htc_batt_info.rep.usb_temp > htc_batt_info.usb_temp_overheat_threshold)) {
				htc_batt_info.rep.usb_overheat = 1;
				pr_debug("%s: USB overheat! pre_usb_temp:%d, usb_temp:%d,"
...
static void batt_force_shutdown_for_batt_protect (void)
{
...
#if (!(defined(CONFIG_MACH_B2_WLJ) || defined(CONFIG_MACH_DUMMY)))
	if (htc_batt_info.rep.batt_id != PROTECTION_M8_SONY_BATT_ID) {
		pr_debug("%s: Not SONY battery\n", __func__);
		return;
	}
#endif
	...
	pr_debug("%s: Level = %d and Voltage = %d.\n",__func__, htc_batt_info.rep.level, batt_vol);
	...
	if (++count >= PROTECTION_COUNT) {
		...
		pr_debug("%s: Set level = 0 because battery voltage low.\n",__func__);
...
static void batt_level_adjust(unsigned long time_since_last_update_ms)
{
	...
	const struct battery_info_reply *prev_batt_info_rep = htc_battery_core_get_batt_info_rep();
	...
	if ((prev_batt_info_rep->charging_source > 0) &&
		...
		pr_debug("cable plug out when level 100, reset timer.\n");
	...
	if ((htc_batt_info.rep.charging_source == 0)
		...
		pr_debug("cable plug out, to store difference between"
	...
	if (is_battery_discharging(prev_batt_info_rep)) {
		if (time_accumulated_level_change < DISCHG_UPDATE_PERIOD_MS
				&& !first) {
			pr_debug("total_time since last batt level update = %lu ms.\n",
					time_accumulated_level_change);
			htc_batt_info.rep.level = prev_level;
		...
		if (is_voltage_critical_low(htc_batt_info.rep.batt_vol)) {
			...
			if (htc_batt_info.decreased_batt_level_check && htc_batt_info.rep.batt_temp > 0)
				batt_check_critical_low_level(&dec_level, htc_batt_info.rep.batt_current);
			else
				dec_level = 6;
			...
			pr_debug("battery level force decreases %d%% from %d%% "
			...
			if ((htc_batt_info.rep.level_raw < 30) || (prev_level - prev_raw_level > 10))
			...
			if (time_since_last_update_ms <= ONE_PERCENT_LIMIT_PERIOD_MS) {
				if (1 <= drop_raw_level) {
					...
					pr_debug("remap: normal soc drop = %d%% in %lu ms."
				...
				pr_debug("remap: phone soc drop = %d%% in %lu ms."
				...
				if (1 <= drop_raw_level) {
					...
					pr_debug("remap: suspend soc drop: %d%% in %lu ms."
			...
			if ((allow_drop_one_percent_flag == false)
					&& (drop_raw_level == 0)) {
				htc_batt_info.rep.level = prev_level;
				pr_debug("remap: no soc drop and no additional 1%%, ui:%d%%\n",
						htc_batt_info.rep.level);
			} else if ((allow_drop_one_percent_flag == true) && (drop_raw_level == 0)
					&& (store_level > 0)) {
				store_level--;
				htc_batt_info.rep.level = prev_level - 1;
				allow_drop_one_percent_flag = false;
				pr_debug("remap: drop additional 1%%. store_level:%d, ui:%d%%\n",
						store_level, htc_batt_info.rep.level);
			} else if (drop_raw_level < 0) {
				if (critical_low_enter) {
					pr_warn("remap: level increase because of exit critical_low!\n");
				}
				store_level += drop_raw_level;
				htc_batt_info.rep.level = prev_level;
				pr_debug("remap: soc increased. store_level:%d, ui:%d%%\n",
						store_level, htc_batt_info.rep.level);
			}
			...
			if (htc_batt_info.rep.level != 100) {
				if ((pre_ten_digit != 10) && (pre_ten_digit > ten_digit)) {
					allow_drop_one_percent_flag = true;
					pr_debug("remap: allow to drop additional 1%% at next level:%d%%.\n",
			...
			if (critical_low_enter) {
				...
				pr_warn("exit critical_low without charge!\n");
			}
			...
			if (htc_batt_info.rep.batt_temp < 0 &&
				...
				pr_debug("remap: enter low temperature section, store_level:%d%%, "
						"dropping_level:%d%%, prev_level:%d%%, level:%d%%.\n",
		...
		if ((htc_batt_info.rep.level == 0) && (prev_level > 1)) {
			htc_batt_info.rep.level = 1;
			pr_debug("battery level forcely report %d%% since prev_level=%d%%\n",
					htc_batt_info.rep.level, prev_level);
		}
	} else {
		if (htc_batt_info.igauge && htc_batt_info.igauge->is_battery_full) {
			...
			if (is_full != 0) {
				...
				if (prev_level > htc_batt_info.rep.level) {
					if (!htc_batt_info.rep.overload) {
						pr_debug("pre_level=%d, new_level=%d, "
					...
					pr_debug("pre_level=%d, new_level=%d, level would use raw level!\n",
							prev_level, htc_batt_info.rep.level);
				}
			}
		}
...
static void sw_safety_timer_check(unsigned long time_since_last_update_ms)
{
	...
	if ((latest_chg_src == HTC_PWR_SOURCE_TYPE_AC) ||
		...
		if (sw_stimer_counter >= HTC_SAFETY_TIME_16_HR_IN_MS) {
			pr_debug("%s sw_stimer_counter expired, count:%lu ms\n",
					__func__, sw_stimer_counter);

			sw_stimer_fault = 1;
			sw_stimer_counter = 0;
		} else {
			pr_debug("%s sw_stimer_counter left: %lu ms\n",
					__func__, HTC_SAFETY_TIME_16_HR_IN_MS - sw_stimer_counter);
                }
	}
}
...
void update_htc_extension_state(void)
{
	...
	if (charger_under_rating && HTC_PWR_SOURCE_TYPE_AC == htc_batt_info.rep.charging_source)
...
static void batt_worker(struct work_struct *work)
{
	...
	pr_debug("%s: total_time since last batt update = %lu ms.\n",
	...
	if ((htc_batt_info.rep.level != 0) && (critical_shutdown ||
		...
		pr_debug("critical shutdown (set level=0 to force shutdown)\n");
		htc_batt_info.rep.level = 0;
		critical_shutdown = 0;
		wake_lock(&batt_shutdown_wake_lock);
		schedule_delayed_work(&shutdown_work,
	...
	if (critical_alarm_level < 0 && prev_chg_src > 0 &&
			htc_batt_info.rep.charging_source == HTC_PWR_SOURCE_TYPE_BATT) {
		pr_debug("critical_alarm_level: %d -> %d\n",
	...
	pr_debug("context_state=0x%x, suspend_highfreq_check_reason=0x%x\n",
			context_state, suspend_highfreq_check_reason);

	if (htc_batt_info.icharger && htc_batt_info.icharger->enable_5v_output) {
		...
		pr_debug("enable_5v_output: %d\n", htc_ext_5v_output_now);
	}

	if (htc_batt_info.icharger && htc_batt_info.icharger->cable_irq_count &&
			htc_batt_info.rep.charging_source > 0)
	...
	if (htc_batt_info.rep.charging_source > 0) {
		...
		pr_debug("prev_chg_src=%d, prev_chg_en=%d, chg_dis_reason/control/active=0x%x/0x%x/0x%x, "
				"chg_limit_reason=0x%x, iusb_limit_reason=0x%x, "
				"pwrsrc_dis_reason=0x%x, prev_pwrsrc_enabled=%d, "
				"context_state=0x%x, htc_extension=0x%x, ftm_charger_control_flag=%d\n",
				prev_chg_src, prev_charging_enabled, chg_dis_reason,
				chg_dis_reason & chg_dis_control_mask, chg_dis_reason & chg_dis_active_mask,
				chg_limit_reason, iusb_limit_reason, pwrsrc_dis_reason, prev_pwrsrc_enabled,
				context_state, htc_batt_info.htc_extension, ftm_charger_control_flag);
		if (charging_enabled != prev_charging_enabled ||
				prev_chg_src != htc_batt_info.rep.charging_source ||
				prev_ftm_charger_control_flag != ftm_charger_control_flag ||
				first || pwrsrc_enabled != prev_pwrsrc_enabled) {
			...
			if (prev_chg_src != htc_batt_info.rep.charging_source ||
					prev_ftm_charger_control_flag != ftm_charger_control_flag || first) {
				pr_debug("set_pwrsrc_and_charger_enable(%d, %d, %d)\n",
				...
				if (pwrsrc_enabled != prev_pwrsrc_enabled) {
					pr_debug("set_pwrsrc_enable(%d)\n", pwrsrc_enabled);
					if (htc_batt_info.icharger &&
							htc_batt_info.icharger->set_pwrsrc_enable)
						htc_batt_info.icharger->set_pwrsrc_enable(pwrsrc_enabled);
				}
				if (charging_enabled != prev_charging_enabled) {
					pr_debug("set_charger_enable(%d)\n", charging_enabled);
					if (htc_batt_info.icharger &&
							htc_batt_info.icharger->set_charger_enable)
						htc_batt_info.icharger->set_charger_enable(charging_enabled);
				}
			}
		}
	} else {
		if (prev_chg_src != htc_batt_info.rep.charging_source || first) {
			...
			pwrsrc_enabled = 0;
			pr_debug("set_pwrsrc_and_charger_enable(%d, %d, %d)\n",
					HTC_PWR_SOURCE_TYPE_BATT, charging_enabled, pwrsrc_enabled);
			if (htc_batt_info.icharger &&
					htc_batt_info.icharger->set_pwrsrc_and_charger_enable)
				htc_batt_info.icharger->set_pwrsrc_and_charger_enable(HTC_PWR_SOURCE_TYPE_BATT,
	...
	if (0 <= critical_alarm_level && critical_alarm_level < critical_alarm_level_set) {
		critical_alarm_level_set = critical_alarm_level;
		pr_debug("set voltage alarm level=%d\n", critical_alarm_level);
		htc_batt_info.igauge->set_lower_voltage_alarm_threshold(
				htc_batt_info.critical_alarm_vol_ptr[critical_alarm_level]);
		if (htc_batt_info.igauge->enable_lower_voltage_alarm)
			htc_batt_info.igauge->enable_lower_voltage_alarm(1);
			...
			unsigned int cmd, unsigned long arg)
{
	...
	case HTC_BATT_IOCTL_UPDATE_BATT_INFO: {
		...
		pr_debug("ioctl: battery level update: %u\n", htc_batt_info.rep.level);
	...
	case HTC_BATT_IOCTL_BATT_DEBUG_LOG:
		if (copy_from_user(htc_batt_info.debug_log, (void *)arg, DEBUG_LOG_LENGTH)) {
			pr_err("copy debug log from user failed!\n");
			ret = -EFAULT;
		}
		break;
	case HTC_BATT_IOCTL_SET_VOLTAGE_ALARM: {
		...
		if (copy_from_user(&alarm_data, (void *)arg,
			...
			pr_err("user set batt alarm failed!\n");
			ret = -EFAULT;
			break;
		}
		...
		pr_debug("Set lower threshold: %d, upper threshold: %d, Enabled:%u.\n",
				alarm_data.lower_threshold,
	...
	case HTC_BATT_IOCTL_SET_ALARM_TIMER_FLAG: {
		...
		if (copy_from_user(&flag, (void *)arg, sizeof(unsigned int))) {
			pr_err("Set timer type into alarm failed!\n");
			ret = -EFAULT;
			break;
		}
		htc_batt_timer.alarm_timer_flag = flag;
		pr_debug("Set alarm timer flag:%u\n", flag);
		break;
	}
	default:
...
#define LTE_GPIO_MBAT_IN (61)
	...
	pr_debug("shut down device due to MBAT_IN interrupt\n");
...
static int htc_battery_prepare(struct device *dev)
{
	...
	if (next_alarm_sec <= 1) {
		pr_debug("%s: passing time:%lu ms, trigger batt_work immediately."
	...
	pr_debug("%s: passing time:%lu ms, alarm will be triggered after %lld sec."
...
static void htc_battery_complete(struct device *dev)
{
	...
	pr_debug("%s: sr_time_period=%lu ms; total passing time=%lu ms. "
	...
	if (htc_batt_timer.total_time_ms >= check_time ||
			(htc_batt_info.state & STATE_WORKQUEUE_PENDING)) {
		htc_batt_info.state &= ~STATE_WORKQUEUE_PENDING;
		pr_debug("trigger batt_work while resume. "
				"(suspend_highfreq_check_reason=0x%x, htc_batt_info.state=0x%x)\n",
				suspend_highfreq_check_reason, htc_batt_info.state);
		htc_batt_schedule_batt_info_update();
	}
...
static struct dev_pm_ops htc_battery_8974_pm_ops = {
		...
		unsigned long event, void *data)
{
	...
	pr_debug("%s: save consistent data\n", __func__);
...
static int htc_battery_probe(struct platform_device *pdev)
{
	...
	htc_battery_core_ptr = kmalloc(sizeof(struct htc_battery_core), GFP_KERNEL);
	if (!htc_battery_core_ptr) {
		pr_err("%s: kmalloc failed for htc_battery_core_ptr.\n", __func__);
		return -ENOMEM;
	}
	...
	htc_battery_core_ptr->func_set_full_level_dis_batt_chg = htc_batt_set_full_level_dis_batt_chg;
	htc_battery_core_ptr->func_context_event_handler = htc_batt_context_event_handler;
	htc_battery_core_ptr->func_notify_pnpmgr_charging_enabled = pdata->notify_pnpmgr_charging_enabled;
	...
	htc_battery_core_ptr->func_trigger_store_battery_data = htc_batt_trigger_store_battery_data;
	htc_battery_core_ptr->func_qb_mode_shutdown_status = htc_batt_qb_mode_shutdown_status;
	...
	htc_batt_info.usb_temp_overheat_increase_threshold = pdata->usb_temp_overheat_increase_threshold;
	htc_batt_info.normal_usb_temp_threshold = pdata->normal_usb_temp_threshold;
	...
	INIT_DELAYED_WORK(&htc_batt_timer.unknown_usb_detect_work, unknown_usb_detect_worker);
#if defined(CONFIG_MACH_B2_WLJ)
	INIT_DELAYED_WORK(&htc_batt_timer.usb_overheat_monitor_work, usb_overheat_monitor);
#endif
	...
	if (rc) {
		pr_err("init kobject htc_batt_timer failed.\n");
	...
	if (rc) {
		pr_err("init kobject htc_cable_timer failed.\n");
		kobject_put(&htc_batt_info.batt_timer_kobj);
		goto fail;
	}

	if (htc_batt_info.icharger && htc_batt_info.icharger->charger_change_notifier_register)
		htc_batt_info.icharger->charger_change_notifier_register(&cable_status_notifier);
	...
	if (!htc_batt_info.batt_fb_wq) {
		pr_err("allocate batt_fb_wq failed\n");
		rc = -ENOMEM;
		goto fail;
	}
	...
	if (ret) {
		pr_err("can't register reboot notifier, error = %d\n", ret);
		return ret;
	}
...
static int __init htc_battery_init(void)
{
	...
	wake_lock_init(&htc_batt_info.vbus_wake_lock,
			WAKE_LOCK_SUSPEND, "vbus_present");
	wake_lock_init(&htc_batt_timer.battery_lock,
			WAKE_LOCK_SUSPEND, "htc_battery_8974");
	...
	wake_lock_init(&voltage_alarm_wake_lock,
			WAKE_LOCK_SUSPEND, "htc_voltage_alarm");
	wake_lock_init(&batt_shutdown_wake_lock,
			WAKE_LOCK_SUSPEND, "batt_shutdown");
