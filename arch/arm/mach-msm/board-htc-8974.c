...
static int64_t htc_8974_get_usbid_adc(void)
{
	...
	if (err < 0) {
		pr_debug("%s: get adc fail, err %d\n", __func__, err);
		return err;
	}
	adc = result.physical;
	adc /= 1000;
	pr_err("%s: chan=%d, adc_code=%d, measurement=%lld, "
			"physical=%lld translate voltage %d\n",
			__func__, result.chan, result.adc_code,
...
static void htc_8974_usb_dpdn_switch(int path)
{
	if (mhl_usb_sw_gpio < 0) {
		pr_debug("%s: no define mhl_usb_sw_gpio\n", __func__);
		return;
	}
...
void htc_8974_cable_detect_register(void)
{
	...
	if (np) {
		...
		pr_debug("%s: no mhl_usb_detect node\n", __func__);
	}
#endif
			...
			struct kobj_attribute *attr, char *buf)
{
	return sprintf(buf, __stringify(EV_KEY) ":"
			__stringify(KEY_BACK) ":241:2007:158:165:"
			__stringify(EV_KEY) ":"
			__stringify(KEY_HOME) ":847:2007:158:165\n");
}
...
static struct of_dev_auxdata htc_8974_auxdata_lookup[] __initdata = {
	...
	OF_DEV_AUXDATA("qcom,qcrypto", 0xFD440000, "qcrypto.0", NULL),
	OF_DEV_AUXDATA("qcom,hsic-host", 0xF9A00000, "msm_hsic_host", NULL),
